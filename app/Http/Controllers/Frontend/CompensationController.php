...
class CompensationController extends Controller
{
    ...
    public function showActiveBet(Request $request)
    {
        $user_id = auth()->user()->id;
        $bet_id = Bet::orderBy('id','DESC')->first()->id;
        $teams = Team::all();

        $bets = Bet::with('moungs')->where('user_id', $user_id)
            ->where('is_finished', 0)->orderBy('id','ASC')
            ->paginate(1);
...
//        dd($bets);
            ...
            $ads = Ad::latest()->first();
        return view($this->rView.'active-bet', compact('bets','ads','teams','bet_id'));
    }

    public function printPrev($printId){
        $user_id = auth()->user()->id;
        $bet_id = Bet::find($printId)->id;
        $teams = Team::all();
        $bets = Bet::with('moungs')->where('user_id', $user_id)
            ->where('is_finished', 0)->orderBy('id','ASC')
            ->where('id',$bet_id)
        ...
        return view($this->rView.'bet-card', compact('bets','teams'));
    }
}
